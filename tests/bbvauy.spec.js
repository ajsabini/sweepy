...
const { extraerCiudades } = require("./utils/ciudades");
const { extraerDepartamentos } = require("./utils/departamentos");
import { extraerBeneficios } from "./utils/tarjetas.js";

test("Scrap bbva gastronomia", async ({ page, browser }) => {
    const baseUrl = "https://www.bbva.com.uy/personas/productos/tarjetas/descuentos/gastronomia.html";
    ...
    const paginationLinks = [baseUrl];

    const hrefs = await page
        .locator(".pagination__items a[href]")
    ...
    for (const url of paginationLinks) {
        ...
        await page.waitForSelector(".editorialcardgrid__cards .card", { timeout: 15000 });

        // Extrae por card lo "barato": hrefs y título
        const rawCards = await page.locator(".editorialcardgrid__cards .card").evaluateAll((cards) =>
            cards.map((card) => {
                // Del card__container: a.card__image[href]
                const img = card.querySelector(".card__container a.card__image img.card__img");
                const imageSrc = img?.getAttribute("src") || img?.currentSrc || null;

                // Del card__content: a[href] que contiene el h3.card__title
                const contentA = card.querySelector(".card__content a[href]");
                const contentHref = contentA?.getAttribute("href") || null;

                // Texto del h3
                const h3 = card.querySelector(".card__content h3.card__title");
                const title = h3?.textContent?.trim() || null;

                //categoria
                const h4 = card.querySelector(".card__content h4.card__preheadline");
                const categoria = h4?.textContent?.trim() || null;

                const tieneEn = (str) => /\ben\b/i.test((str ?? "").replace(/\u00A0/g, " "));

                let categoriaLocal = "";

                if (tieneEn) {
                    categoriaLocal = categoria ? categoria.split(/\ben\b/i)[0].trim() : "";
                } else {
                ...
                return { imageSrc, contentHref, title, categoria, categoriaLocal };
            })
        );

        // Convierte a absolutas usando la URL actual (por si cambia el host/base)
        const absoluteCards = rawCards.map((c) => ({
            imageSrc: c.imageSrc ? new URL(c.imageSrc, url).href : null,
            contentHref: c.contentHref ? new URL(c.contentHref, url).href : null,
            title: c.title,
            categoria: c.categoria,
            categoriaLocal: c.categoriaLocal,
        }));

        results.push(...absoluteCards);
    }

    console.log(results);

    for (const r of results) {
        const href = r?.contentHref;
        if (!href) continue;
        ...
        r.vigencia_text = vigenciaText;
        r.vigencia_iso = vigenciaISO;
        //-* vigencia
        ...
        r.descuentos = descuentos;
        const descuentoPorTarjeta = extraerBeneficios(descuentos);
        r.descuentoPorTarjeta = descuentoPorTarjeta;
        //-* descuentos
        ...
        r.locales = locales;
        r.direccion = direccionAntesDeComa(locales?.[0] ?? null);
        r.cuidades = extraerCiudades(locales || []);
        r.departamentos = extraerDepartamentos(locales || []);
        //-*locales
        ...
        r.credito = creditSegments.map((seg) => ({
        ...
        r.credito_by_card = r.credito.reduce((acc, seg) => {
